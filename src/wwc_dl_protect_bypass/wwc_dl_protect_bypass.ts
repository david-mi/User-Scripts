(async function init() {
  ...
  async function getHtmlElementsAsync(querySelector: string) {
    let elements = document.querySelectorAll(querySelector) as NodeListOf<HTMLElement>
    let tries = 0
    const MAX_TRIES = 100

    while (elements.length === 0 && tries < MAX_TRIES) {
      elements = await new Promise<NodeListOf<HTMLElement>>((resolve) => {
        setTimeout(() => {
          tries += 1
  ...
  if (location.host.includes("dl-protect")) {
    const unProtectedLinkElement = document.querySelector<HTMLLinkElement>("#protected-container a[rel='external nofollow']")
    ...
    for (const linkRow of linkRows) {
      const hostname = (linkRow.children[1] as HTMLElement).innerText
      ...
      if (hostname === "1fichier") {
        ...
        window.addEventListener("retrieved-link", (event) => {
          ...
          if (event instanceof CustomEvent) {
            dlLinkElement.href = event.detail as string
            dlLinkElement.innerText = event.detail as string
          }
        })
      }
    }

    function getDlLinkElement(linkRow: HTMLElement) {
      return linkRow.
        firstElementChild!
        .querySelector("a")!
    }

    function createDlLinkIframe(dlLinkHref: string) {
