...
(async function () {
    ...
    let videosContainers = [];
    let videosContainersWithoutShorts = [];
    async function getVideosContainers() {
        let newVideosContainers = Array.from(document.querySelectorAll("ytd-rich-item-renderer"));
        let tries = 0;
        const MAX_TRIES = 10;
        while (newVideosContainers.length === videosContainersWithoutShorts.length && tries < MAX_TRIES) {
            newVideosContainers = await new Promise((resolve) => {
                setTimeout(() => {
                    tries += 1;
    ...
    videosContainersWithoutShorts = Array.from(document.querySelectorAll("ytd-rich-item-renderer"));
