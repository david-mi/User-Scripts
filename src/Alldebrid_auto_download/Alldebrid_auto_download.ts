...
(async function () {
  ...
  async function getLinksTextArea() {
    ...
    while (linksTextArea === null && counter <= MAX_TRIES) {
      ...
      linksTextArea = await new Promise((resolve) => {
        setTimeout(() => {
          resolve(document.querySelector<HTMLTextAreaElement>("#links"));
        }, 200);
      });
    }
  ...
  async function handlePaste() {
    const fullLinkRadioInput = document.querySelector<HTMLInputElement>("input[type='radio'][value='link']")
    const startProcessButton = document.querySelector<HTMLLinkElement>("#giveMeMyLinks");
